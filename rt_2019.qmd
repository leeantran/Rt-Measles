...
p_agegr <- ggplot(df_2019, aes(x = dates, fill = agegr)) +
...
plot_rt <- function(data){
  ...
  ggplot(df_plot, aes(x = dates)) +
    ...
    annotate(
    "segment", x = as.Date("2018-10-17"), xend = as.Date("2018-10-17"),
    y = 4.5, yend = 0, color = "black", size = 1) +
  geom_rect(aes(xmin = as.Date("2018-10-17"), xmax = as.Date("2019-01-01") + 15, ymin = 2.5, ymax = 4.5), fill = "#42CAFD") +
  annotate("text", x = as.Date("2018-10-17") + 1, y = 3.5, label = "Vaccination",
           color = "black",
           hjust = 0, size = 12 / .pt)
}
```
...
Theo USCDC, giai đoạn **R~t~** của Sởi năm 2018-2019 diễn ra như sau [@cdc2024a]:
