...
**Đến ngày 17/10** xu hướng lây nhiễm của Sởi tại các số quận/huyện như sau:

-   Huyện Bình Chánh: số ca nhiễm đang giảm R~t~ = 0,65 (0,44 - 0,91).

-   TP. Thủ Đức: số ca nhiễm đang tăng R~t~ = 1,51 (1,15 - 1,93).

-   Quận 8: số ca nhiễm đang tăng Rt = 2,74 (1,54 - 4,29).

-   Huyện Củ Chi: số ca nhiễm đang tăng R~t~ = 1,12 (0,73 - 1,59).

-   Quận Bình Tân: số ca nhiễm có thể tăng R~t~ = 0,89 (0,61 - 1,22).

-   Quận 12: số ca nhiễm ổn định R~t~ = 2,07 (1,16 - 3,24).

-   Quận Tân Phú: số ca nhiễm ổn định R~t~ = 3,28 (2,05 - 4,78).

-   Huyện Hóc Môn: số ca nhiễm có thể giảm R~t~ = 1,74 (1,05 - 2,60).

-   Các Quận/Huyện còn lại không ước tính được hệ số lây Rt (do không đủ dữ liệu).

**Trong tuần từ 11/10 - 17/10,** dựa theo giá trị và khoảng tin cậy của R~t,~ xu hướng lây nhiễm của Thành phố và Quận/Huyện như sau:

-   Thành phố: R~t~ vẫn lớn hơn 1 với khoảng 99% khoảng tin cậy của R~t~ \> 1, do đó số ca nhiễm **đang tăng**.

-   Quận/Huyện: có 4 quận/huyện vẫn có R~t~ lớn hơn 1 (với trên 90% khoảng tin cậy của R~t~ \> 1 **(màu tím)** bao gồm: Hóc Môn, TP. Thủ Đức, Quận 8, Quận 12 và Tân Phú. Củ Chi có số ca nhiễm **ổn định**, Quận Bình Tân đang ở giai đoạn số ca nhiễm **có thể giảm** và Bình Chánh có số ca nhiễm **đang giảm (với dưới 10% KTC của R~t~ > 1 bắt đầu từ 12/10 đến hiện tại).**

**So với 1 tuần trước:**

-   Quận 12 và Tân Phú **tăng** báo động về số ca nhiễm **(từ màu cam thành màu tím)**.

-   Quận Bình Tân **giảm** báo động về có số ca nhiễm **(từ màu đỏ thành màu đỏ thành màu vàng)**.

-   Quận 8 và TP. Thủ Đức vẫn giữ nguyên mức độ báo động số ca nhiễm **đang tăng (màu tím)**.

-   Hóc Môn **tăng** báo động về số ca nhiễm **(từ màu vàng thành màu tím).**

-   Củ Chi **giảm** báo động về số ca nhiễm **(từ màu tím thành màu cam)**.

-   Bình Chánh **giảm** báo động về số ca nhiễm **(từ màu tím thành màu xanh)**.
...
## Load data
df <- readRDS("df_clean2010.rds")
df <- filter(df, dates <= "2024-10-18")
```
...
p_hist <- ggplot(df_new, aes(x = week)) +
    ...
    scale_x_continuous(breaks = seq(20, 42, by = 1), limits = c(20, 42))
