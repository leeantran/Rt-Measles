...
-   Năm 2024, dữ liệu phân tích từ ngày 04/03/2024 đến ngày 08/10 có tổng cộng 210 ca bệnh bị **thiếu dữ liệu** về **ngày khởi phát*,*** do đó báo cáo sử dụng **ngày nhập viện** thay thế để ước tính **R~t~**.
...
plot_rt <- function(data){
  ...
  ggplot(df_plot, aes(x = dates)) +
    ...
    scale_x_date(date_labels = "%b %d", date_breaks = "1 week",
                 limits = c(ymd("2024-05-27", ymd("2024-10-05")))) +
...
p_hist <- ggplot(df_filter, aes(x = dates)) +
    ...
    scale_fill_manual(values = "#87A2FF") +
    scale_color_manual(values = "#E78F81") + 
    scale_x_date(date_labels = "%b %d", date_breaks = "1 week", limits = c(ymd("2024-05-27", ymd("2024-10-05"))))
...
-   Từ ngày 22/07 - 09/09: số ca nhiễm **đang tăng (R~t~** = 1.1, KTC: 0.95 - 1.29 vào ngày 09/09 có thể thấy KTC trên của **R~t~** còn trên 1 do đó số ca bệnh có thể tăng).

-   Từ ngày 10/09 - 15/09: số ca nhiễm **có thể tăng (màu đỏ)**.

-   Từ ngày 17/09 - 19/09: số ca nhiễm **có thể giảm (màu vàng**.

-   Từ ngày 20/09 - 28/09: số ca nhiễm **ổn định (màu cam)**.
...
p_hist <- ggplot(df_complete, aes(x = week)) +
    geom_histogram(aes(y = I), fill = "#87A2FF", stat = "identity", binwidth = 1.5, width = 2) +
    labs(x = NULL, y = "Incidence", fill = NULL, color = NULL) +
    theme_classic() +
    theme(axis.text.x = element_text(angle = 60, hjust = 1, size = 10),
          axis.text.y = element_text(size = 10),
...
# Kết hợp dữ liệu
map_data_date <- vn_qh_filter %>%
  left_join(df_qh_date, by = c("NAME_2" = "qh"))

# Vẽ bản đồ 
p <- ggplot(data = map_data_date) +
  geom_sf(aes(fill = case), color = "white", size = 0.3) +  
  scale_fill_gradient2(low = "blue", mid = "white", high = "red", midpoint = 5,  # Màu sắc
                         na.value = "gray90") + 
  geom_sf_text(aes(label = NAME_2), size = 3, color = "black") +
  labs(fill = "Số ca mắc", title = "Ngày: {frame_time}") +  
  theme_minimal() +  
  theme(axis.text = element_blank(), 
        axis.ticks = element_blank(),
        panel.grid = element_blank()) +
  ease_aes('linear') +  # Hiệu ứng chuyển động
  transition_time(dates) +
  shadow_mark(past = TRUE, future = FALSE)

# anim <- animate(p, nframes = 50, fps = 2)
```

![](map_animation.gif){ width=100% }
